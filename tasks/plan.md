...
- [ ] 마이그레이션과 엔티티에 정적 낭독 키, 캐릭터 음성 설정, 답변 음성 키·상태를 추가한다.
- [ ] 공유 계약에 낭독 URL과 답변 음성 상태·URL을 추가한다.
- [ ] 공식 동화 시드에 이미 업로드된 14개 낭독 키를 연결한다.
- [ ] SQL·metadata·시드 정합성 테스트를 추가한다. 실제 DB에는 적용하지 않는다.

**Checkpoint:** 백엔드 build/test에서 스키마·계약이 일치하고 DB 쓰기 명령을 실행하지 않았다.

### Step 2 — 정적 낭독 API

- [ ] 본편 페이지 조회가 `narrationAudioUrl`을 발급한다.
- [ ] 비하인드 A/B 조회가 각 결과의 `narrationAudioUrl`을 발급한다.
- [ ] 오디오 키 누락과 서명 실패가 페이지 응답을 실패시키지 않는 테스트를 추가한다.

**Checkpoint:** 본편 5개와 A/B 두 개의 URL 매핑 및 `null` 폴백이 서비스 테스트에서 확인된다.

### Step 3 — 캐릭터 답변 TTS 백엔드

- [ ] `TextToSpeechPort`와 `OpenRouterTextToSpeechAdapter`, `OPENROUTER_TTS_MODEL` 환경변수 검증을 추가한다. 인증은 기존 `OPENROUTER_API_KEY`를 공유한다.
- [ ] 답변 완료 뒤 음성을 선점·생성·업로드·저장한다.
- [ ] 음성 전용 재시도 API와 최신 상태 조회를 추가한다.
- [ ] 동시 요청, 공급자 실패, 업로드 실패, 서버 중단 상태, 완료 음성 재사용을 테스트한다.

**Checkpoint:** 같은 채팅의 TTS 호출은 한 번이며 실패해도 저장된 답변 텍스트가 유지된다.

### Step 4 — 공통 내레이션 UX

- [ ] 리더 범위 단일 오디오 컨트롤러와 `NarrationPlayer.tsx`를 추가한다.
- [ ] 시연 `BookReader`와 체험 리더 양쪽에 같은 조작·상태 UI를 연결한다.
- [ ] 현재/다음 음성 및 5쪽의 A/B 프리로드, 탭 숨김·화면 이탈 정지를 구현한다.
- [ ] 최초 입력, 자동재생, 일시정지 의미, 종료 시 쪽 유지, 누락 음성 동작을 Vitest로 검증한다.

**Checkpoint:** 내레이션 하나만 재생되고 페이지·분기 이동 시 Q1~Q12, Q15~Q17, Q24~Q27이 고정된다.

### Step 5 — 캐릭터 답변 음성 UX

- [ ] `useCharacterChat`에 오디오 상태 폴링과 음성 전용 재시도를 연결한다.
- [ ] `CharacterChat`에 준비·재생·실패 UI를 추가하고 사용자 질문은 읽지 않는다.
- [ ] 새 답변만 자동재생하고 저장 답변은 수동재생하는 규칙을 구현한다.
- [ ] 채팅 조작 시 내레이션 정지와 전역 배타성을 검증한다.
...
- [ ] 운영 환경변수와 캐릭터 voice 설정 누락을 배포 전 게이트로 보고한다.
- [ ] 사용자 승인 후에만 마이그레이션·시드 실행, 커밋·푸시·PR을 진행한다.

## Tests / Verification

- [ ] Backend: migration/entity, adapter, story service, after-story, chat service/controller tests
- [ ] Frontend: audio controller, narration UI, reader integration, chat audio states tests
