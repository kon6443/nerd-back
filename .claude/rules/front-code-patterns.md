...
> 최종 확인일: **2026-09-14** · 규모: `app/`+`components/`+`lib/` 70파일 · `scripts/` 3 · 테스트 11(75건).
...
| 위계 | 모양 | 쓰는 곳 |
|---|---|---|
| `primary` | 비취색(`primary-strong`, 2026-09-18) | 한 묶음에서 가장 중요한 동작 **하나** |
| `secondary` | 흰색 | 그다음 동작 |
| `dark` | 초록빛 검정(`--color-night`) | secondary 인데 흰 면 위라 묻힐 때 |
| `tertiary` | 텍스트 | 되돌아가기·임시 도구처럼 눈에 덜 띄어야 할 때 |

- 주 색은 **누를 수 있는 것**의 신호다 — 선택 칩·진행 표시도 `primary`/`accent-a`(비취 계열)를 쓰고, 🚫 풍경·빈 삽화 같은 **장식에는 쓰지 않는다**(`hill-*` 세이지). 🚫 **노랑·보라 버튼 금지.**
...
### 화면이 여럿인 라우트는 화면마다 파일을 나눈다

`viewState` 로 갈리는 화면(제작 현황 · 비하인드 선택 · 완독)은 **각자 파일**이다. 한 파일에 모으면
980줄이 되어 어느 상태를 고치는지 찾는 데만 시간이 든다(2026-09-14 분리, 980 → 748).

- 나눈 화면은 **상태를 갖지 않는다.** 폴링·재시도·전환은 리더가 소유하고 화면은 받은 것만 그린다 —
  화면이 자기 폴링을 가지면 리더의 것과 두 벌이 되어 요청이 배로 나간다.
- 상태 전환은 **콜백 이름으로** 받는다(`onSkip` · `onRestart` · `onBackToStory`). 🚫 `setViewState` 를
  그대로 내려보내지 않는다 — 화면이 부모의 상태 기계를 알게 되면 그 화면만 따로 옮길 수 없다.
- 🚫 라우트 밖(`components/`)으로 올리지 않는다. 한 라우트만 쓰는 화면이다.

### 반복되는 클래스 문자열은 상수로 (`READER_BAR`)

책 위아래 줄처럼 **같은 문자열이 3곳 이상** 나오면 상수로 뽑는다. 특히 그 안에 **빠뜨리면 버그가 되는
토큰**(`relative`)이 있으면 반드시 그렇게 한다 — 실제로 빠뜨려 넘어가는 종이가 조작줄을 덮었다.

### 리더 — 넘김과 대화 (Slice 7, 2026-09-14)

- ⭐ **쪽 넘김은 `BookPager` 하나다.** 시연과 체험이 같은 것을 쓴다. 🚫 넘김 마크업·애니메이션을
  화면 쪽에 다시 적지 않는다 — 두 벌이 되면 한쪽만 바뀐다.
  - `BookPager` 는 **쪽 번호 → 내용**(`renderArt`/`renderText`)만 받는다. 데이터를 어디서 가져오는지
    알게 하는 순간 두 리더 중 하나는 못 쓰게 된다.
  - ⚠️ **도착 쪽 내용이 넘김 시작 시점에 있어야 한다.** 넘어가는 종이 뒷면이 곧 도착 쪽이라, 쪽을
    바꾼 뒤에 받으면 빈 종이가 700ms 돈다. 체험 리더는 `pagesToPrefetch` 로 인접 쪽을 미리 받는다.
- ⭐ **1단(모바일)은 낭독 조작이 책 프레임 아래 밖이다**(2026-09-18). `BookPager` 가 본문 쪽 안·책 밖 두 자리에 그리고
  CSS 가 폭에 따라 하나만 보인다. 🚫 JS 로 화면 폭을 읽어 한 곳만 그리지 않는다 — 서버 HTML 과 달라져 첫 화면에서 튄다.
- ⭐ **1단 넘김은 책등(왼쪽) 축 180°** 다. 넘어가는 면(`.faceStack`)은 밑면 쪽과 **픽셀 단위로 같아야** 한다(삽화 비율·흰 테두리).
  다르면 넘김 시작 순간 종이 복제본이 떼어지는 것처럼 보인다. 이전 쪽은 `out` 을 시간만 뒤집은 `in` 이다.
- ⭐ **대화 상태는 리더가 소유한다**(`useCharacterChat`). 🚫 대화 컴포넌트 안에 두지 않는다 —
  표면을 닫는 순간 언마운트되어 **답변 폴링이 끊긴다.** 질문 기회는 이미 썼는데 답을 못 본다.
- 모달은 네이티브 `<dialog showModal()>` 이다. 포커스 트랩·Esc·top-layer·배경 inert 를 브라우저가 준다.
  🚫 직접 구현하거나 라이브러리를 넣지 않는다. 배경 스크롤 잠금만 `globals.css` 의 `body:has(dialog[open])` 이 맡는다.
- 🚫 **`AppHeader` 에서 리더 쿼리를 읽지 않는다.** 몰입 여부는 리더가 `<html data-reader>` 를 세우고
  CSS 가 고른다 — 헤더에서 `useSearchParams` 를 쓰면 **전 화면에 Suspense 경계**가 생긴다.
- ⚠️ `SPREAD_QUERY`(48rem)는 `readerNav.ts` 가 소유한다. `BookFrame.module.css` 의 미디어쿼리와
  **같은 값이어야 한다** — 넘김 모양과 대화 dock 가능 여부가 이 값으로 갈린다.
- ⭐ **넘김은 마디(`.seg`)를 중첩해 만든 곡면이다.** 단일 요소를 `rotateY` 하면 그림자·질감을 아무리
  얹어도 판때기가 돈다(2026-09-14 실측). 🚫 마디 구조를 단일 평면으로 되돌리지 않는다.
- ⭐ **자르는 층과 3D 가 도는 층을 나눈다.** 정적인 내용은 둥근 모서리로 잘려야 하고(`.surface`),
  들린 종이는 원근으로 커지므로 잘리면 안 된다(`.book`). 한 상자에서 둘을 다 하려고
  `overflow-clip-margin` 으로 경계를 물리면 잘림은 없어지지만 **둥근 모서리까지 밀려나 직각이 된다**
  (2026-09-14 실측 — 그래서 층을 나눴다).
- ⚠️ 책 밖으로 솟는 종이가 UI 를 덮지 않게, 책은 `isolation: isolate` 하고 **헤더·조작줄에 `relative`**
  를 준다. positioned 형제가 static 형제를 덮는다는 규칙 때문이다.
- ⚠️ **마디의 첫 조각에는 가로 위치를 명시한다**(`inset-inline-start/end: 0`). 비워 두면
  `position: absolute` 가 부모 왼쪽으로 떨어뜨려, **축이 오른쪽인 방향에서만** 회전축이 어긋난다 —
  한쪽 넘김만 이상해서 원인을 찾기 어렵다(2026-09-14).
- 🚫 곡면 음영을 **마디마다** 그리지 않는다. 마디는 회전하며 화면 폭이 달라져 그라데이션 기울기가
  경계에서 꺾이고, 그것이 세로 줄무늬로 보인다. **종이 전체 폭(`.sliceInner`)에 한 번** 그린다.
- 🚫 **CSS 가 조용히 죽는 세 가지** — 셋 다 빌드·타입·테스트를 통과하고 **화면에서만** 안 나온다:
  - `animation` shorthand 를 **이름 없이** 쓰지 않는다. 길이가 0 이 되어 시작과 끝이 같은 프레임에 온다.
  - `linear-gradient(to var(--x), …)` — `to` 뒤의 var 는 파싱되지 않는다. `--dir: to right` 처럼 **토큰째** 넘긴다.
  - `color-mix(… calc(var(--n) * 1%) …)` — 백분율에 계산식을 넣으면 규칙째 무시된다. `30%` 처럼 **단위째** 넘긴다.
  - ⚠️ 그래서 **시각 변경은 브라우저로 확인한다.** 빌드 산출 CSS 에 문자열이 있는 것은 적용의 근거가 아니다.
- ⏳ `readerOptions.ts` 와 `ReaderPreviewSettings` 는 **한시적**이다. 후보를 고르면 지운다
