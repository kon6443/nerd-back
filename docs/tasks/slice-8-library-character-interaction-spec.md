...
> 상태: **구현·피드백·말풍선 핀 시각 고도화·전체 검증 완료 (2026-09-16)**
> 확정일: 2026-09-15
...
7. 단일 통 이미지에서 실루엣 윤곽을 정확히 표현하기 어려운 한계를 피해, 캐릭터 우측 상단의 **부유형 마법 말풍선 핀**을 항상 노출하고 호버·포커스 시 이름 캡슐로 확장한다.
...
- 평상시에도 보이는 부유형 말풍선 핀과 호버·포커스 시 확장되는 이름 캡슐
- 말풍선 핀은 캐릭터 얼굴을 가리지 않도록 히트박스 우측 상단 밖에 둔다
...
새 화면을 만들지 않고 기존 그림책의 종이·마법 톤을 확장한다. 말풍선 핀은 기존 `paper`, `gold`, `gold-strong` 토큰과 소프트 그림자를 사용한다. 새 색 리터럴은 컴포넌트에 추가하지 않는다.
...
- 구현을 막는 제품 결정은 없다.
- 실루엣 마스크가 없는 통 이미지의 한계는 말풍선 핀 방식으로 해소했으며, 별도 마스크 생성 작업은 열지 않는다.
- 완성 동화의 `← 제작 현황 보기` 노출 문제는 `isAllCompleted`/`completed` 가드로 해결했다. 구체적인 추가 결함이 없으므로 포괄적인 상태 머신 리팩토링은 후속 작업으로 남기지 않는다.
