...
| 구현 브랜치 | **구현은 별도 브랜치에서.** 이 문서·규약은 계획 브랜치에 남긴다 | **09-04** |
...
## 두 가지 모드 — 무엇을 먼저 만들지를 결정한다

| | 시연 (Demo) | 체험하기 (Trial) |
|---|---|---|
| 목적 | 관심 유도 — "시식 코너" | 실제 기능 |
| 로그인 | 불필요 | **필요** |
| 얼굴 | 미리 합성해 둔 가상 인물 사진 | 실제 촬영 → 합성 |
| 낭독 | 팀 녹음 파일 | 팀 녹음 파일 |
...
**시연 모드는 AI·스토리지·인증 없이 전부 만들 수 있다.** 필요한 것은 "사전 제작된 페이지를 순서대로 내려주는 API"뿐이고,
그건 **Slice 1 에서 이미 만들었다.** 미리 합성된 가상 인물 이미지는 그 동화의 `base_image_key` 다 — 코드가 아니라 콘텐츠다.
...
| # | 항목 | 상태 | 무엇이 정해지면 풀리는가 |
|---|---|---|---|
| **D1** | 오브젝트 스토리지 | 열림 | **용도가 바뀌었다** — 얼굴이 아니라 **생성 결과 이미지 보관**용이다. 선택 기준에 「서명 URL 지원」을 넣는다 |
...
| **D6** | 프론트 테스트 프레임워크 | 열림 | 프론트는 테스트가 **의도적으로 미도입**이다(`ci:core` 에 `test` 단계 없음). 그런데 팀 방침은 TDD 다. 도입할지 결정해야 한다 |
...
1. **체험 횟수 2~3회 제한** — 1인당 상한을 만드는 가장 큰 장치 (Slice 2)
...
| 건당 300원 미달 | 횟수 제한 + 레퍼런스 재사용 + 캐시 + 재시도 상한. 모델 확정 시 재계산 |
