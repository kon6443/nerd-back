...
> 작성일: 2026-09-04
> 상태: **Slice 0~8 구현·검증 완료 (2026-09-16).** 남은 것은 D10(탈퇴·책 삭제 시 DB+스토리지 제거) 구현과 리더 `보기 설정` 채택/제거 결정이다.
...
| 얼굴 데이터 | **영구 저장하지 않는다.** `DIRECT_FACE_MODE=true`에서는 생성 성공까지 `temp/source-photo/`에만 임시 보관하고, 실패·중단 시에도 24시간 내 lifecycle로 폐기한다 | 09-02 · **09-18** [`상세 명세`](data-deletion-temporary-photo-spec.md) |
...
| ~~D3~~ | ~~이미지 생성 모델~~ | **✅ 해소 (09-08)** | **Port & Adapter (MockImageAdapter 기본 + OpenRouterImageAdapter 병행).** `slice-3-face-session-spec.md` |
| ~~D4~~ | ~~비하인드 분기 구조~~ | **✅ 해소 (09-10)** | 본편 5쪽 + 이미지 없는 A/B 분기 UI + A/B별 결과 6쪽. 공통 5장과 결과 2장을 선생성하며, 최초 선택만 보존한다. [`slice-6-behind-branch-spec.md`](slice-6-behind-branch-spec.md) |
| ~~**D5**~~ | ~~콘텐츠 저작권·라이선스~~ | **✅ 현재 범위에서 종료 (09-16)** | 퍼블릭 도메인 원작을 바탕으로 내용을 AI로 재창작하고 삽화도 AI로 제작한다는 프로젝트 가정을 채택했다. 외부 번역문·삽화·음원을 추가하면 그 자산의 권리는 별도로 재확인한다. |
| ~~D6~~ | ~~프론트 테스트~~ | **✅ 해소 (09-04)** | **vitest 도입.** 순수 로직과 `lib/api` 부터. `ci:core` 에 `test` 단계가 생긴다 |
| ~~**D10**~~ | ~~계정·데이터 삭제~~ | **✅ 정책·상세 명세 확정 (09-18) · 구현 필요** | 책 삭제·탈퇴 시 개인 DB와 오브젝트를 제거한다. 직접 얼굴 모드의 실사는 `temp/source-photo/`에만 임시 보관하고 완료 즉시 삭제하며, 24시간 TTL은 안전망이다. 상세: [`data-deletion-temporary-photo-spec.md`](data-deletion-temporary-photo-spec.md) |
...
| ~~**D5** 저작권~~ | ~~실 콘텐츠 투입~~ | **✅ 현재 AI 재창작 범위에서 종료 (09-16)** |
| ~~**D9** 오디오~~ | ~~낭독·캐릭터 답변 TTS~~ | **✅ 해소 (09-14)** — Slice 7 상세 설계 확정 |
| ~~**D10** 계정·데이터 삭제~~ | ~~없음 (정책)~~ | **✅ 정책 확정, DB+스토리지 삭제 구현 필요** |

**현재 구현 슬라이스를 막는 미결정은 없다.** 다음 백로그는 D10을 코드로 강제하는 책 삭제·계정 탈퇴 구현이다.
...
| `IconButton` | 리더·비하인드의 네비게이션 | 생성 안 함 | 실제 사용처는 링크이므로 `ActionLink`로 충분 |
| `ActionButton` | 주요 CTA | 생성 안 함 | 공유 `actionClass` variant로 해결 |
| `FaceGuideCircle` | 얼굴 촬영 | 대체 완료 | `CaptureStudio`가 단일 정면 사진 촬영·첨부 UX를 소유 |
...
| `ChatSurface` · `CharacterHotspots` | 체험 리더의 등장인물 대화 | ✅ (2026-09-16) | 하단 `ChatLauncher`는 제거하고 삽화 위 말풍선 핀이 진입점을 소유. 상태는 `useCharacterChat`이 소유 |
| `NarrationPlayer` | 리더 | ✅ (2026-09-16) | 첫 사용자 입력 후 페이지별 정적 MP3를 재생한다. [`slice-7-tts-spec.md`](slice-7-tts-spec.md) |
| `GeneratingView` | 생성 대기 | ✅ | 페이지별 진행률·팁·부분 실패·재시도 UX를 소유 |
...
🚫 **얼굴 사진 바이트를 담는 테이블은 없다.** DB에는 객체 키만 둔다. `DIRECT_FACE_MODE=true`의 임시 원본 키(`sourcePhotoKey`)와 보관 규칙은 [`실사 임시 보관 및 사용자 데이터 삭제 명세`](data-deletion-temporary-photo-spec.md)를 따른다.
...
### C안 (2026-09-18 이전 기본 운영 모드: `DIRECT_FACE_MODE=false`)

```text
1단계 (동기, 짧다)   정면 사진 1장 ──► 주인공 레퍼런스 이미지 1장
...
1. **당시 얼굴 보유 시간 = 단일 요청 수명.** 디스크에 닿지 않으므로 "저장하지 않는다"가 구조로 보장된다
...
- 이 결론은 `DIRECT_FACE_MODE=false`에만 적용된다. 현재 확정한 `DIRECT_FACE_MODE=true` 정책은 실사를 `temp/source-photo/`에 임시 보관하며, 생성 성공 시 즉시 삭제하고 실패·중단 시 24시간 lifecycle로 폐기한다. [`상세 명세`](data-deletion-temporary-photo-spec.md)가 우선한다.
...
### 현재 구현의 보관 규칙 (2026-09-16 확인)

- 브라우저는 촬영 결과를 `Blob`과 미리보기 URL로만 잠시 보유하고 제출·교체·화면 이탈 시 해제한다.
- 백엔드는 Multer 메모리 버퍼로 정면 사진을 받는다. DB에 사진 바이너리나 base64를 저장하지 않는다.
- 기본값 `DIRECT_FACE_MODE=false`에서는 실사를 OpenRouter 레퍼런스 생성에 사용한 뒤 메모리에서 해제하고, 스토리지에는 AI가 만든 주인공 레퍼런스만 저장한다.
- **현재 로컬 `apps/back/.env`는 `DIRECT_FACE_MODE=true`다.** 이 모드에서는 AI 1차 변환을 건너뛰고 사용자의 실사 정면 사진을 레퍼런스 객체로 스토리지에 저장한다. DB에는 그 객체의 `reference_image_key`만 저장한다.
- 위 내용은 2026-09-16의 **현행 구현**이다. 2026-09-18 확정 정책은 `DIRECT_FACE_MODE=true`도 유지하되 실사를 `temp/source-photo/`에만 임시 보관하고, 전체 생성 완료 뒤 즉시 삭제하는 방식으로 변경한다. 상세 기준은 [`data-deletion-temporary-photo-spec.md`](data-deletion-temporary-photo-spec.md)를 따른다.
...
| 삭제 규칙 | **✅ 정책·상세 명세 확정 (2026-09-18)** — 개인 생성물은 책 삭제·탈퇴 시 제거. 직접 얼굴 모드 실사는 `temp/source-photo/`에 임시 보관 후 완료 즉시 삭제하고 24시간 TTL을 안전망으로 둔다. 상세: [`data-deletion-temporary-photo-spec.md`](data-deletion-temporary-photo-spec.md) |
...
> 이 체크리스트는 Slice 3 착수 전의 운영 준비 기록이다. 2026-09-16 문서 정리에서 외부 운영 상태는 재검증하지 않았으며, 현재 구현 백로그로 취급하지 않는다.
...
| 3 | `GET /stories/:slug/pages/:pageNo` | 한 쪽만 필요할 때 (리더는 쓰지 않는다) | ✅ 구현됨 |
| 3-A | **`GET /stories/:slug/pages`** | **리더 — 본편 전 쪽을 한 번에.** 진입 1회로 끝나고 쪽을 오갈 때 추가 요청이 없다 | ✅ 구현됨 (2026-09-14) |
...
| 8 | `POST /sessions` | 세션 생성·미완료 세션 재사용 | ✅ 구현됨 |
| 9 | `POST /sessions/:id/face` | 정면 사진 → 레퍼런스 생성 (동기) | ✅ 구현됨 |
| 10 | `POST /sessions/:id/personalize` | 페이지 개인화 시작 (202) | ✅ 구현됨 |
| 11 | `GET /sessions/:id/pages` | 진행 상태 폴링 | ✅ 구현됨 |
| 12 | `POST /sessions/:id/pages/:n/retry` | 실패 페이지 재시도 | ✅ 구현됨 |
| 13 | `POST /sessions/:id/pages/:n/characters/:role/messages` | 등장인물 대화 | ✅ 구현됨 |
| 14 | `GET /sessions/:id/after-story` · `POST .../choice` | 비하인드 + A/B | ✅ 구현됨 |
...
| 첫 진입 · 서재 | 공통 | 1 |
| 시연 리더 | 시연 | 2, 3 |
| 로그인 · 가입 | 체험 | 4, 5, 7 — `/login` 화면에서 로그인/가입 모드를 전환 |
| 나의 동화 앨범 | 체험 | 7 |
| 얼굴 촬영 | 체험 | 8, 9 |
| 생성 대기 | 체험 | 10, 11, 12 |
| 개인화 리더 | 체험 | 3, 11 |
...
| Port | 용도 | 상태 |
|---|---|---|
| `LlmPort` | 등장인물 대화 | ✅ `StoryChatLlm` OpenRouter 구현 |
| `ImageGenerationPort` | 얼굴 → 레퍼런스, 레퍼런스 → 페이지 | ✅ Mock·OpenRouter 어댑터 구현 |
| `StoragePort` | 레퍼런스·생성 결과 보관 | ✅ Local·S3 어댑터 구현 |
...
- [x] **프론트**: 서재 목록 → 상세 → 시연 리더(`BookFrame`) + **전역 네비(`AppHeader`)**. 낭독 UI는 Slice 7에서 구현 완료
- [x] **스키마 결손 보강** — 분기 키·최초 선택 기록은 Slice 6, 개인화 썸네일은 Slice 8에서 반영
...
### Slice 3 — 세션 + 얼굴 → 레퍼런스 · **완료 (09-08)**

> 상세 설계서: [`docs/tasks/slice-3-face-session-spec.md`](slice-3-face-session-spec.md)

- [x] `packages/contracts` — `CreateSessionRequestSchema`, `CreateSessionResponseSchema`, `UploadFaceResponseSchema`
- [x] `story_sessions` 엔티티 + 마이그레이션 — **`UNIQUE(user_id, template_id)`**
- [x] API 8 (`POST /sessions`) — 이미 완성된 동화면 **409**. 미완료 세션(`draft`/`failed`)은 기존 세션을 재사용하여 재촬영/재시도 보장
- [x] API 9 (`POST /sessions/:id/face`) — `multipart/form-data` 정면 1장 필수 + 좌/우 선택 (1~3장 유연 지원), 크기 상한(장당 5MB), **매직바이트** 검증, 레퍼런스 생성 즉시 원본 폐기
- [x] `ImageGenerationPort` — `MockImageAdapter` (로컬 0원/0에러 기본 활성화) + `OpenRouterImageAdapter` (환경변수 스위칭)
- [x] `StoragePort` + **S3 어댑터** (`@aws-sdk/client-s3`) / **로컬 어댑터** — 업로드 · 서명 URL 발급. 🚫 키는 UUID. 개발 데이터는 `dev/` prefix 로 운영 데이터와 격리
...
### ✅ Slice 4 — 개인화 파이프라인 (완료 2026-09-08)
...
- [x] **프론트 UI/UX**: 페이지별 진행률, 폴링 백오프, 부분 실패 표시·재시도 버튼 구현

### ✅ Slice 5 — 등장인물 대화 (구현·TTS 확인 완료 2026-09-16)

- [x] `LlmPort` OpenRouter 어댑터와 현재 장면·persona 기반 프롬프트
- [x] 응답 길이·질문 횟수 상한, 예약·폴링·실패 복구
- [x] 응답·음성 저장과 본문·내부 프롬프트 미노출
- [x] **프론트**: 캐릭터 히트박스, 대화 UI, 본편 복귀, 답변 TTS
- [x] 검증: 단위·E2E·프론트 테스트와 실제 TTS 동작 확인

### ✅ Slice 6 — 비하인드 + A/B (완료 2026-09-10)
...
- [x] 상세 설계·수용 기준: [`slice-6-behind-branch-spec.md`](slice-6-behind-branch-spec.md)

---

### ✅ Slice 7 — 리더 표면 통일 + 대화 분리 (구현 완료 2026-09-14)

시연 리더는 09-13~14 에 하드커버 표지·종이 단면·3D 넘김을 갖췄는데 **체험 리더는 밋밋한 `BookFrame`
그대로다.** 유저가 자기 얼굴로 만든 결과물이 시연보다 초라하다 — 이번 슬라이스가 그 격차를 없앤다.

**두 리더의 차이는 디자인이 아니라 쪽 데이터 공급 방식이다.** 시연은 서버가 전 쪽을 한 번에 주고,
체험은 본문·삽화·비하인드가 각각 다른 API 에서 온디맨드로 온다. 넘김은 넘어가는 종이 앞뒷면에
떠나는 쪽과 도착 쪽을 **동시에** 그려야 성립하므로, 넘김 엔진을 데이터 소스와 떼는 것이 핵심이다.

| 결정 | 내용 | 근거 |
|---|---|---|
| 넘김 엔진 | `BookPager` 로 추출 — **쪽 번호 → 내용**만 받고 데이터 출처를 모른다 | 두 리더가 같은 넘김을 쓰되 데이터 경로는 각자 소유 |
| 제어 방식 | 목표 쪽을 **prop 으로 받는 제어형**. 주소로 옮길지(시연) 상태로 옮길지(체험)는 부르는 쪽이 정한다 | 시연의 `pushState` 를 체험에 강요하지 않는다 |
| 본문 프리페치 | 체험 리더가 **인접 쪽(±1)** 을 미리 받는다. 순차 로드는 유지 | 도착 쪽 본문이 없으면 종이 뒷면이 700ms 빈다. 동시 호출은 429 |
| 대화 표면 | 인라인 섹션 → **플로팅 런처 + 시트**. 「다음 페이지」는 책 하단 조작줄이 단독 소유 | 인라인 섹션이 있는 한 책이 화면을 채울 수 없다 |
| 대화 상태 | `useCharacterChat` 훅을 **리더가 소유**해 런처·시트에 함께 내린다 | 시트를 닫아도 `pending` 폴링이 살아 답변이 도착한다 |
| 모달 구현 | 네이티브 `<dialog showModal()>` | 포커스 트랩·Esc·top-layer·배경 inert 를 브라우저가 준다. 🚫 새 의존성 금지 |
| 전역 헤더 숨김 | `<html data-reader="immersive">` + CSS | 🚫 `AppHeader` 에서 `useSearchParams` 를 읽지 않는다 — 전 화면에 Suspense 경계가 생긴다 |

#### 평가 옵션 — 쿼리 두 축 (한시적) ⏳

디자인을 **실제로 써 보고 고르기로 했다**(2026-09-14 지시). 후보를 전부 만들어 URL 쿼리로 즉시
전환한다. 화면 안 「보기 설정」 위젯이 `history.replaceState` 로 쿼리만 바꾼다 — 저장소(localStorage)를
쓰지 않는 이유는 값을 읽는 순간 첫 렌더가 한 번 바뀌어 깜빡이기 때문이다(`AppHeader` 의 `data-session` 과 같은 문제).

| 축 | 값 | 기본값 |
|---|---|---|
| `chat` | `sheet`(모바일 바텀 · 데스크톱 우측) · `modal`(중앙) · `dock`(데스크톱 밀어내기) | `sheet` |
| `immersive` | `on`(헤더 숨김 + 화면 채움) · `off`(현재 레이아웃) | `on` |

- 위젯은 **조건 없이 항상 보인다.** `?preview=1` 같은 노출 분기를 두지 않는다 — 예외가 곧 버그다.
- 🚫 시연 리더에는 붙이지 않는다. 대화가 없고 몰입이 고정이라 **조절할 축이 0개**다.
- ⚠️ `dock` 만 열고 닫을 때 책 폭이 변해 펼침면과 단면 두께가 다시 잡힌다. 넘김이 도는 동안 토글을 잠근다.
- **회수 조건**: 조합이 정해지면 쿼리 두 축·위젯·고르지 않은 껍데기를 **별도 커밋으로 지운다.**
  선택값이 상수가 된다. 🚫 이 항목이 남아 있는 한 이 슬라이스는 완료가 아니다.

- [x] `BookPager` 추출 + 시연 리더 이관 (2026-09-14)
- [x] 체험 리더에 적용 + 본문 프리페치 (`pagesToPrefetch` + vitest 5건)
- [x] 대화 훅 추출 → 런처 + 시트 3안 → 인라인 섹션 제거
- [x] 몰입 축 + 보기 설정 위젯 (`parseReaderOptions` + vitest 6건)
#### 2차 피드백 반영 (2026-09-14)

| 지적 | 대응 |
|---|---|
| 플로팅 버튼·보기 설정이 **구석이라 눈에 안 들어온다** | 둘을 **화면 아래 가운데** 한 줄로 모았다. 위치는 리더의 하단 바 컨테이너가 **혼자** 소유하고 버튼은 모양만 갖는다 — 각자 `fixed` 를 달면 안전영역·겹침 계산이 두 벌이 된다. 런처는 `compact` → 기본 크기 |
| 넘김 질감이 **판때기 같다** | ① 종이 섬유 그레인(SVG `feTurbulence` 타일, 곱하기 합성)을 본문·삽화·넘김 면 전부에 깔았다 — **이것이 가장 큰 한 가지다.** ② 들린 면이 아래에 **그림자를 던진다**(`leaf-cast`). ③ 그늘만 있던 자리에 **휨 하이라이트**를 함께 넣었다(어두운 띠 + 밝은 띠). ④ 회전 중 `scaleX(0.955)` 로 살짝 좁아진다 — 뻣뻣한 판은 폭이 변하지 않는다 |

- 🚫 삽화 면의 종이 결은 `::before` 다. `::after` 는 넘어가는 면(`.face::after`)이 쓰고 있어 서로 지운다.
- ⚠️ 그레인 `baseFrequency` 를 낮추면 얼룩이 커져 종이가 아니라 대리석이 된다.

#### 3차 피드백 반영 — 실제 화면으로 확인 (2026-09-14)

| 지적 | 대응 |
|---|---|
| 버튼이 **너무 구석**이라 안 보인다 | 「보기 설정」과 런처를 **화면 아래 가운데** 한 줄로. 좁은 화면에서는 「보기 설정」이 아이콘만 남아 조작줄을 덮지 않는다 |
| 질감이 아니라 **넘김 동작이 뻣뻣하다**(2차에서 잘못 알아들었다) | 종이를 **마디 6개로 접어 곡면**을 만든다(`BookPager` 의 `LeafSegments`). 마디를 중첩해 회전이 누적되고, `--curl` 이 0→1→0 이라 평평하게 출발해 휘었다가 눕는다. 가운데가 많이 꺾이는 C자 배분 + 마디별 명암 |

**실측으로 잡은 것** — 셋 다 `ci:all` 을 통과하고 화면에서만 안 나왔다. 상세는 [`lessons.md`](../lessons.md) 2026-09-14.

1. 이름 없는 `animation` shorthand → 길이가 0 이 되어 넘김이 시작과 동시에 끝났다
2. `linear-gradient(to var(--x))` → 파싱되지 않아 곡면 음영이 통째로 사라졌다
3. `color-mix(… calc(var() * 1%) …)` → 같은 이유로 그늘이 나오지 않았다

**확인한 경로**(로그인 `test` → 「잭과 콩나무」 리더, Chrome 1280×800 · 400×820):
정방향·역방향 넘김 곡면, 모바일 1단, 하단 바 겹침 없음. ⚠️ headless 는 `prefers-reduced-motion` 을 켠 것으로
보고하므로 드라이버에서 꺼야 넘김이 보인다.

#### 4차 피드백 — 넘김이 아직 어색하다 (2026-09-14)

**원인은 곡면을 만든 방식 자체였다.** 전체 회전량을 마디가 **나눠 갖게** 했더니, 180° 를 6마디가
30° 씩 나눠 후반에 **종이가 원통으로 말렸다**(실측 프레임에서 S자로 감겼다). 곡률이 회전량에 비례해
커지는 구조였던 것이다.

| 고친 것 | 내용 |
|---|---|
| **회전과 휨을 분리** | 몸통(`.leaf`)이 `--turn-angle` 만큼 통째로 돌고, 마디는 `--bend`(몇 도)만 얹는다. 휨의 크기가 회전량과 무관해져 후반에도 말리지 않는다. 🚫 `--bend` 의 **합은 0** 이다 — 그래야 종이 끝이 몸통과 같은 방향을 향해 평평하게 눕는다 |
| 마디 6 → **8** | 4~6 에서는 텍스트 면의 마디 경계에 계단이 보였다 |
| 속도 820 → **640ms**, 이징 `linear` | 속도 곡선을 **키프레임이** 만든다(눌리자마자 들리고, 중간이 가장 빠르고, 94% 에서 4° 지나쳤다 되돌아와 착지). 이징과 키프레임을 겹치면 구간마다 두 번 휘어 울퉁불퉁해진다 |
| 그림자를 **종이 전체에 하나만** | 마디마다 걸었더니 이웃 위에 그림자를 드리워 **세로 줄무늬 8줄**이 생겼다 |
| 마디를 **1px 겹침** | 정확히 이어 붙이면 마디마다 안티앨리어싱 이음새가 남는다. 자르는 겹도 둘(`.segFace` + `.slice`)에서 하나로 줄였다 |

**확인**: 1280×800 정방향·역방향, 400×820 1단. 프레임을 `document.getAnimations()` 로 고정해 관찰했다.
⚠️ 캡처가 리더의 안전 타이머(1400ms)보다 오래 걸리면 종이가 치워진 뒤를 찍게 된다 — 드라이버에서
**1초 이상 `setTimeout` 만 막고** 찍는다.

#### 5차 피드백 (2026-09-14)

| 지적 | 진단 · 대응 |
|---|---|
| 좌·우 넘김이 **대칭이 아니다** / **속도가 다르다** | **애니메이션은 대칭이다** — 실측으로 양방향 `animation-duration` 0.64s 동일, 마디 변환 행렬이 부호만 반대임을 확인했다. 다르게 보이는 것은 **넘어가는 면의 내용**이다: 다음 쪽은 오른쪽 **본문**(흰 바탕)이, 이전 쪽은 왼쪽 **삽화**(디테일 가득)가 넘어간다. 책 구조상 불가피하다. 다만 체감 차이의 한 축이던 **첫 넘김 지연**은 프리페치로 없앴다 |
| 페이지 넘길 때 **세로줄** | 명암을 **마디마다** 그리던 것이 원인. 마디는 회전하며 화면 폭이 달라져 **그라데이션 기울기가 경계에서 꺾인다.** → `.sliceInner`(종이 한 장 전체 폭)에 **한 번만** 그려 마디가 자기 구간을 잘라 쓰게 했다. 값도 기울기도 저절로 이어진다. 조각 폭도 `calc((100% - 2px) * N)` 으로 보정(`.slice` 가 좌우 1px 넓다) |
| 종이가 **반대로 구부러진다** | `SEGMENT_BEND` 부호를 뒤집었다. 넘기는 손은 종이 뒤에서 미므로 가운데가 **보는 사람 쪽으로** 부풀어야 한다 |
| 페이지가 바뀔 때 부자연스럽다 | ① 본문 프리페치를 **전 쪽**으로 확장(현재 → 다음 → 이전 → 나머지, 순차 유지). ② 첫 삽화가 뜨면 **나머지 삽화 전부** 프리로드. ③ 종이 그림자를 `--curl` 에 묶어 **시작·끝에 0** — 고정 그림자는 종이가 걷히는 순간 그늘만 사라져 화면이 튀었다 |

#### 6차 피드백 — 이전 쪽 넘김 + 전 쪽 조회 API (2026-09-14)

| 지적 | 진단 · 대응 |
|---|---|
| 이전 쪽 넘김이 **종이를 옮기는 것 같지 않다** | **실제 버그였다.** 마디의 첫 조각에 가로 위치를 주지 않아 `position: absolute` 가 그것을 부모 왼쪽으로 떨어뜨렸다. 다음 쪽은 축이 왼쪽이라 우연히 맞았지만 **이전 쪽은 축이 오른쪽이라 마디들이 축 반대편에서 시작**했다 — 회전축이 책등이 아니었다. `.leaf[data-turn] > .seg` 에 `inset-inline-start/end: 0` 을 못 박았다. 같은 300ms 시점 종이 폭이 230px(≈66°) → 50px(≈85°) 로, 실제 회전량과 일치하게 됐다 |
| 이전 쪽으로 갈 때 **그림이 없다** | 데이터 문제가 아니었다 — 넘김 도중 밑면 1장·종이 8장 모두 `naturalWidth > 0` 이고, 1→2→3→2→1 을 걸어도 빈 이미지·"불러오는 중" 이 없다. 이 책은 모든 장이 `[왼쪽 삽화 \| 오른쪽 본문]` 고정이라 **다음 쪽은 종이 뒷면에 그림이, 이전 쪽은 글이** 온다. 실제 책과 같은 구성이라 **그대로 두기로 했다**(2026-09-14 결정) |

#### 7차 피드백 — 성경책 얇은 종이처럼 (2026-09-14)

**지적**: "너무 두꺼운 종이가 넘어가는 것 같다. 성경책 얇은 종이처럼."

**진단**: 휨이 **가운데만 대칭으로 부푸는** 모양(누적 최대 22°, 합 0)이라 끝이 늘 몸통과 같은 방향을 향했다 —
판이 약간 휜 채 **통째로** 돈다. 얇은 종이는 책등 쪽이 먼저 들리고 **바깥 끝이 늦게 끌려오다가 마지막에
앞질러 넘어간다.** 거기에 183° 되돌아오는 착지·진한 그림자·790ms 가 무게를 더했다.

| 바꾼 것 | 내용 |
|---|---|
| 🔄 **4차의 "휨 합 0" 규칙을 뒤집는다** | `SEGMENT_BEND` 를 사인 활(합 0)에서 **끝으로 갈수록 커지는 경사**(합 +58°)로. `--curl` 이 **부호를 가진다**: 들릴 때 + (끝이 뒤처짐) → 넘어가며 − (끝이 앞지름) → 착지 0. 평평하게 눕는 목적은 **착지 시 `--curl` 0** 이 지킨다 |
| 휨을 **몸통 키프레임이 함께 소유** | 마디별 `seg-curl` + 지연을 없앴다. 끝이 뒤처지는 양은 **몸통 각도보다 작아야** 종이가 책 아래로 파고들지 않는다 — 두 값이 한 키프레임에 있어야 그 제약을 지킬 수 있다 |
| 790 → **620ms**, 되돌아오는 착지 제거 | 가벼운 종이는 튕기지 않고 사뿐히 눕는다 |
| 그림자·명암 **절반 이하** | 얇은 종이는 빛이 투과해 그늘이 옅다 |
| **비침** | 휘는 동안 종이가 최대 12% 투명해져 밑면 글자가 비친다(`.slice` 의 opacity — 🚫 `.leaf`·`.seg` 에 주지 않는다. opacity 는 `preserve-3d` 를 평면으로 만든다) |

#### API 3-A — 본편 전 쪽 조회 (계약 추가)

**왜**: 삽화는 이미 `GET /sessions/:id/pages` 로 전 쪽을 한 번에 받는데 **본문만 쪽 단위**였다. 그 비대칭
때문에 프론트에 "어느 쪽을 먼저 받을지" 정하는 우선순위 로직과 429 를 피하는 순차 루프가 생겼다 —
**API 모양이 만들어 낸 코드**다.

| | 전 | 후 |
|---|---|---|
| 체험 리더 진입~완독 본문 요청 | 5회 | **1회** |
| 시연 리더 방문 1회 | 상세 1 + 쪽 N | **2회** (어느 쪽으로 들어오든 같다) |
| 백엔드 쿼리 | 쪽마다 3회 | **3회 고정** (템플릿 · 쪽 · 등장인물 `In(pageIds)`) |
| 프론트 | `prefetch.ts` + 순차 루프 | **삭제** |

- 🚫 비하인드(`branchKey` 'a'/'b')는 넣지 않는다 — 세션의 선택 결과라 `after-story` 가 소유한다.
- 단건 `GET /pages/:pageNo` 는 **호환을 위해 남긴다.** 리더는 쓰지 않는다.
- ⚠️ 라우트는 **목록을 단건보다 앞에** 둔다.
- 마이그레이션 없음(조회만 추가).

**실측**(dev, 새 브라우저로 로그인 → 리더 진입 → 6번 넘김):
진입 시 `stories/:slug/pages` **1회**, 이후 6번 넘기는 동안 **본문·삽화 요청 0**.
⚠️ dev 는 StrictMode 가 이펙트를 두 번 돌려 상세·대화 조회가 2회씩 찍힌다(프로덕션 1회). `output: standalone`
이라 `next start` 로는 프로덕션 실측이 안 된다 — **프로덕션 요청 수는 미검증**이다.

**남은 것**: 쪽을 넘길 때마다 `GET /sessions/:id/pages/:n/chat` 이 1회 나간다(쪽마다 남은 질문 수가
달라 필요한 조회다). 이것까지 한 번에 받으려면 세션 대화 상태 목록 API 가 필요하다 — **별건**.

#### 7차 피드백 — 더 종이처럼 · 책 위가 잘림 (2026-09-14)

| 지적 | 대응 |
|---|---|
| 아직 **판때기** 같다 | 휨을 **사인 활**로 다시 뽑았다 — 누적 각도가 `-18° × sin(π·x)` 를 그리도록 마디 각도를 계산한다(`SEGMENT_BEND`). 마디 8 → **10**. `--curl` 도 곧바로 1 로 세우지 않고 **0 → 0.7 → 1 → 0.62 → 0** 으로 부풀렸다 편다 |
| 넘길 때 종이 **윗부분이 잘린다** | `.book` 의 `overflow: hidden` 이 원인. 들린 종이는 원근으로 커지는데 책 테두리가 잘라냈다. → **`overflow: clip` + `overflow-clip-margin: 140px`** (앞줄 `hidden` 은 미지원 브라우저 폴백). 평소에는 아무것도 그 밖으로 나가지 않아 **넘기는 순간에만** 효과가 보인다 |
| (딸린 문제) 솟은 종이가 조작줄을 덮음 | `.volume` 에 `isolation: isolate` + 헤더·조작줄에 `relative`. 책이 positioned 라 static 인 형제는 그 아래로 깔린다 — 종이는 책 위의 물건이고 버튼은 화면에 떠 있는 것이다 |

**진폭 실측**: 14° = 판때기 · 28° = 종이가 너무 말려 폭이 줄고 90° 부근에서 앞뒤 면이 오래 섞임 ·
**18° 가 그 사이**. 하이라이트도 24% → 15% 로 낮췄다(글자를 지울 만큼 셌다).

#### 8차 피드백 — 모서리 회귀 · "텅, 쿵" (2026-09-14)

| 지적 | 대응 |
|---|---|
| 종이 **모서리가 직각**이 됐다 | 7차의 `overflow-clip-margin` 이 원인이었다 — 경계를 밖으로 물리면 **둥근 모서리까지 함께 밀려난다.** → **층을 나눴다**: `.book`(원근 · 종이가 솟는 층, 자르지 않음) > `.surface`(테두리 · 모서리 · 바탕, `overflow: hidden`). 정적 내용은 둥글게 잘리고 넘어가는 종이는 `.surface` 밖이라 잘리지 않는다 |
| **판때기 하나가 텅, 쿵** 하며 움직인다 | ① 마디가 **저마다 8ms 씩 늦게** 휜다(`--depth` × 8ms). 전부 같은 순간에 휘면 곡면이어도 한 덩어리가 통째로 움직인다 — 늦게 따라오면 힘이 종이를 타고 흘러 **출렁인다.** ② 착지의 오버슛을 4° → 3° 로 줄이고 되돌아오는 구간을 6% → 10% 로 늘렸다(급히 튕기면 그것이 "쿵" 이다) |

- ⚠️ 몸통 길이는 **지연 총합보다 길어야 한다** — 640ms + (10마디 × 8ms) = 712ms 이므로 `.leaf` 는 720ms.
  짧으면 끝 마디가 휜 채로 종이가 걷힌다.
- 실측: `seg0` 0ms … `seg9` 72ms, 몸통 0.72s.

#### 9차 피드백 — 넘길 때 모서리 · 아직 뻣뻣 (2026-09-14)

| 지적 | 대응 |
|---|---|
| **넘길 때** 모서리가 직각 | 8차에서 종이(`.leaf`)를 `.surface` 밖으로 뺐더니 **책의 둥근 모서리를 물려받지 못했다.** → 종이의 **바깥 끝 마디**(`.segTip`)에만 같은 곡률을 준다. 책등 쪽은 원래 각지고, 마디 사이를 굴리면 틈이 벌어진다. 곡률은 `.book` 의 `--tip-top/--tip-bottom` 이 소유해 하드커버(1.25/0.5rem)와 기본 책(1rem)이 갈린다. ⚠️ 자르는 것은 `.slice` 이므로 radius 도 거기에 준다 |
| 아직 종이처럼 부드럽지 않다 | **각속도를 재 보니 거의 등속이었다**(80ms 당 17 → 24 → 24°). 종이는 그렇게 움직이지 않는다. ① 속도 곡선을 **느긋한 시작 → 중반 급가속 → 착지**로(20% 15° · 55% 110° · 80% 168° · 92% 183° · 100% 180°). ② 마디 간 시간차 8 → **14ms**(몸통 720 → 790ms). ③ 휨 진폭 18 → **22°** |

**남은 한계**: 종이의 **위아래 가장자리는 직선**이다. 지금은 세로로만 마디를 나눠서 가로 방향 휨이 없다.
가로까지 나누면 마디가 10×N 개가 되어 비용이 급증한다 — 더 필요해지면 그때 판단한다.

#### 리팩터링 — 전수조사 후 적용 (2026-09-14)

반복 패턴을 기계적으로 훑고(`className` 문자열 빈도 · 파일 크기 · 블록 경계) **이득이 분명한 것만** 적용했다.

| 적용 | 내용 | 근거 |
|---|---|---|
| 화면 3개 분리 | `GeneratingView`(155) · `BranchView`(135) · `EndView`(70) — `page.tsx` **980 → 748줄** | 한 파일에 5개 상태가 인라인이라 고칠 자리를 찾기 어려웠다. 나눈 화면은 상태를 갖지 않고 콜백(`onSkip`·`onRestart`·`onBackToStory`)으로만 전환한다 |
| `READER_BAR` 상수 | 책 위아래 줄 클래스 4곳 → `BookPager` 가 소유 | 그 안의 `relative` 를 빠뜨리면 **넘어가는 종이가 조작줄을 덮는다** — 실제로 겪은 버그라 한 곳에 모았다 |
| 미사용 import 제거 | `NextImage` · `Badge` (분리로 안 쓰이게 됨) | lint 경고 2 → 0 |

**적용하지 않은 것**(오버엔지니어링 판단):

- `{ code: SUCCESS_CODE, data, message: '' }` 6곳 — 전역 인터셉터를 두지 않는 것이 이 저장소의 결정이다(`CLAUDE.md`).
- `ApiCommon*` 데코레이터 14회 — 이미 추출된 상태. 더 묶으면 라우트별 응답이 가려진다.
- `text-xl font-bold text-ink` 류 5회 — Tailwind 유틸 조합이라 상수화 이득보다 간접 참조 비용이 크다.
- CSS 의 `[data-turn="next"]`/`[data-turn="prev"]` 대칭 규칙 — 방향이 반대라 논리적으로 하나로 못 줄인다.

**회귀 검증**(브라우저, 1280×800): 리더 → 제작현황(진행률 7/7) → 비하인드(선택 2개) → **5쪽 복귀**
→ 완독(표지 1개) → **리더 복귀(1/6)**. 콜백 3개 모두 동작, `pageerror` 0건.

#### 배포에서만 보이던 삽화 깜빡임 (2026-09-14)

**증상**: 배포 환경에서 이전 쪽으로 갈 때 삽화가 깜빡인다. 로컬에서는 재현되지 않았다.

**원인**: 밑면 삽화(`baseArt`)가 도착 쪽으로 **교체되는 순간 그 이미지가 아직 없으면** `<img>` 가
빈 프레임을 낸다. 교체 시점이 방향마다 다르다 — `baseArt = nextSpread ? turn.from : turn.to`.

| | 밑면 삽화 | 교체 시점 |
|---|---|---|
| 다음 쪽 | 떠나는 쪽 유지 → 넘김 후 도착 쪽 | 넘김이 **끝난 뒤** |
| 이전 쪽 | 처음부터 도착 쪽 | 넘김 **시작 즉시** |

이전 쪽이 더 눈에 띈 이유는 빈 구간의 길이가 아니라 **위치**다(실측은 오히려 다음 쪽이 길었다).
"넘어간 뒤 로딩"은 자연스럽게 읽히지만 "넘기자마자 그림이 사라졌다 돌아오는 것"은 깜빡임으로 읽힌다.

**로컬에서 안 보인 이유**: 삽화가 캐시 히트라 네트워크 응답조차 발생하지 않는다(응답 로그에 favicon 만
잡혔다). 배포는 원격 스토리지에서 1366px 이미지를 받는다.

**고친 것 — 둘 다 필요하다**:

- **A. 프리로드를 화면 전환 직후로** — 전에는 화면에 그려진 첫 삽화의 `onLoad` 를 기다린 뒤에야
  나머지를 받기 시작했다. 배포에서는 그 지연이 몇 초다. 이제 `sessionPages` 를 받고 리더로 넘어가는
  즉시 전 쪽을 받는다(`onImageLoad` 경로와 `preloadRemainingArtwork` 는 제거).
- **B. 도착 삽화가 올 때까지 직전 삽화를 아래에 남긴다**(`.artHold`) — 네트워크는 보장할 수 없으므로
  A 만으로는 창이 항상 남는다. ⭐ **로드 완료를 감지하지 않는다**: 두 장을 겹쳐 두면 위 장이 로드되기
  전에는 아무것도 그리지 않아 아래 장이 보이고, 로드되는 순간 자연히 덮인다.

**실측**(캐시 정상 · 이미지 첫 로드 +2000ms · 프리로드 완료 전 조작 · 같은 측정 기준으로 대조):

| | 다음 쪽 | 이전 쪽 |
|---|---|---|
| 수정 전 | 빈 프레임 **28** (450ms) | 빈 프레임 **3** (28ms) |
| 수정 후 | **0** | **0** |

⚠️ 재현할 때 `Network.setCacheDisabled` 를 켜면 안 된다 — 배포에도 브라우저 캐시는 있고, 그걸 끄면
`.artHold` 의 직전 삽화까지 다시 받게 되어 **없는 문제를 만든다**(그렇게 오측해서 한 번 헛짚었다).

- [x] **화면 회귀 확인** — 후속 Slice 8 수동 QA에서 시연·체험 리더와 반응형 화면 확인
- [ ] **별도 제품 결정** — 평가용 `보기 설정` UI와 `chat`/`immersive` 쿼리를 정식 기능으로 채택할지, 기본값으로 고정하고 제거할지 선택한다. Slice 7 구현 완료를 막지 않고 제품 정리 항목으로 분리한다.

---

### ✅ Slice 8 — 제작형 서재와 삽화 속 캐릭터 대화 (완료 2026-09-16)

- [x] 완성 동화의 개인화 1쪽 썸네일과 제작 모드 CTA 분기
- [x] 본편·6A·6B 삽화 히트박스에서 기존 대화 표면 열기
- [x] 타원형 광원 시안을 부유형 마법 말풍선 핀으로 대체·고도화
- [x] 완성 세션의 `← 제작 현황 보기` 노출 결함 해결, 포괄적 상태 머신 리팩토링은 구체적 결함이 없어 종료

상세 명세와 검증 기록은 [`slice-8-library-character-interaction-spec.md`](slice-8-library-character-interaction-spec.md)와 [`../../tasks/todo.md`](../../tasks/todo.md)가 소유한다.

---

### 주 색 — 짙은 올리브 → 비취색 (2026-09-18 요청)

"Primary 가 너무 칙칙해 잘 안 보인다." #62 가 전체를 숲 톤으로 통일하며 주 버튼이 `#354f38`(거의 검정에 가까운 올리브)가 됐다.

| 결정 | 내용 | 근거 |
|---|---|---|
| 주 색 | **비취색(jade)** — `primary #1f9a74` · `primary-strong #0f7a57`(버튼·강조 글자) · 립 `primary-deep #0a5a40` | 같은 숲 계열이라 3D 홈·세이지 풍경과 어울리면서 채도로 눈에 띈다. 흰 글자 대비 5.24:1(AA) |
| 기각한 후보 | 산호색 — 가장 잘 보이지만 오류색(빨강)·빨간 모자 표지와 겹친다. 파랑 — #62 의 숲 톤 통일과 갈라진다. 연두 — 가장 흔해 동화나라다움이 약하다 | 네 후보를 실제 화면(홈·서재)에 입혀 비교했다 |
| 진행·선택 표시 | `accent-a` 도 같은 비취 계열로 맞춘다 | 진행 막대·선택 칩은 주 동작과 같은 색이라는 기존 규칙 |
| 장식은 그대로 | 풍경·빈 삽화 자리·책 장면의 나무는 `hill-*` 세이지를 쓴다 | 주 색은 **누를 수 있는 것**의 신호다. 장식까지 물들면 신호가 흐려진다 |

### 모바일 리더 다듬기 (2026-09-18 요청)

| 지적 | 결정 | 근거 |
|---|---|---|
| 모바일에서 「읽어주기·처음부터」가 책 안 스크롤에 숨는다 | 모바일(1단)은 낭독 조작을 **책 프레임 아래 밖**에 둔다. 펼침면(48rem↑)은 지금처럼 본문 쪽 안 | 1단의 본문 칸은 15rem 고정 스크롤이라 끝에 붙인 조작은 스크롤해야만 보였다(390px 실측: 칸 240px · 내용 526px) |
| 시연 리더 이전·다음 크기가 다르고 아래 설명이 풍경 위라 안 읽힌다 | 조작줄을 **흰 고정 하단바**로. 이전·다음은 같은 폭, 설명은 바 안 | 체험 리더 하단바와 같은 모양 |
| 모바일 넘김이 "종이가 뜯겨나가는" 느낌 | ① 넘기는 종이의 모양을 밑면 쪽과 **같게**(삽화 비율·흰 테두리·모서리) ② 1단에서는 비침 없음 ③ 90°에서 멈춰 사라지지 않고 책등을 축으로 **180° 넘어간다**(뒷면은 빈 종이) ④ 이전 쪽은 떠나는 쪽이 오른쪽으로 날아가는 대신 **앞 쪽이 왼쪽에서 넘어와 덮는다** | 실측 프레임: 넘김 시작 순간 삽화가 262→198px 로 줄고 본문이 튀어 올라 **종이 복제본이 떼어지는** 모양이었다. 90° 착지는 종이가 가장 느린 순간에 모서리째 사라졌다 |
...
| 프론트 | `pnpm front ci:core` → PR 직전 `pnpm front ci:all` | lint · 타입 · test · 빌드 |
