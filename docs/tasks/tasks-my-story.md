...
| 얼굴 데이터 | **영구 저장하지 않는다.** 실사 사진 직접 합성 단일 모드로 운영하며, 생성 성공까지 `temp/source-photo/`에만 임시 보관하고 완료 시 즉시 삭제하며 실패·중단 시에도 24시간 내 lifecycle로 폐기한다 | 09-02 · **09-18** [`상세 명세`](data-deletion-temporary-photo-spec.md) |
...
🚫 **얼굴 사진 바이트를 담는 테이블은 없다.** DB에는 객체 키만 둔다. 실사 임시 원본 키(`sourcePhotoKey`)와 보관 규칙은 [`실사 임시 보관 및 사용자 데이터 삭제 명세`](data-deletion-temporary-photo-spec.md)를 따른다.
...
### 실사 사진 직접 합성 단일 모드 (표준화 확정)

> **2026-09-18 확정**: 과거 C안(AI 모델로 1차 캐릭터 레퍼런스를 생성하던 '일반 모드')은 얼굴 닮음 훼손 및 대기 시간(15~30초) 문제로 **완전 폐지**되었으며, 환경변수 `DIRECT_FACE_MODE` 또한 시스템 단일 동작으로 통합되어 제거되었다. 동화 생성은 사용자의 실사 정면 사진을 동화 속 장면에 직접 합성하는 **실사 단일 모드**로만 진행된다.

```text
1단계 (동기, 짧다)   정면 사진 1장 ──► temp/source-photo/ 임시 업로드 (sourcePhotoKey만 저장)
2단계 (비동기)       실사 원본 ──────► 페이지 6장 개인화 합성
                     └─ 전체 완료 즉시 ──► 실사 원본 객체 삭제 (sourcePhotoKey NULL)
```

1. **얼굴 보관 최소화**: 실사 원본은 생성 중에만 `temp/source-photo/`에 존재하며, 완료 시 즉시 삭제되고 실패 시 24시간 TTL로 안전하게 폐기된다.
2. **품질 및 일관성**: 실제 얼굴 사진의 특징을 각 템플릿 장면에 직접 반영하여 얼굴 유사도가 크게 향상된다.
3. **대기 시간 단축**: 1차 캐릭터 생성 대기(15~30초)가 제거되어 사진 업로드 즉시 다음 단계로 넘어간다.

**결론**
- 동화 생성은 **실사 사진 직접 합성 단일 모드**로 표준화된다.
- 얼굴은 `multipart/form-data`로 1장을 받아 `temp/source-photo/`에 보관한다.
- 임시 원본 객체에는 외부 서명 URL을 발급하지 않는다 (`referenceImageUrl: null`).
- 얼굴 검증(크기·매직바이트 기반 실제 형식)은 수신 즉시 수행한다.
- 🚫 얼굴 바이트를 로그·에러 메시지·APM에 싣지 않는다.
- 상세 기준은 [`data-deletion-temporary-photo-spec.md`](data-deletion-temporary-photo-spec.md)를 따른다.
