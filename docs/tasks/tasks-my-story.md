...
| ~~**D10**~~ | ~~계정·데이터 삭제~~ | **✅ 정책 확정 (09-16) · 구현 필요** | 책 삭제 시 해당 세션의 DB 세션·분기·대화와 레퍼런스 이미지·개인화 삽화·캐릭터 답변 MP3를 함께 제거한다. 탈퇴 시 사용자의 모든 책에 같은 규칙을 적용한 후 계정을 제거한다. 공유 템플릿·정적 낭독 자산은 사용자 데이터가 아니므로 제외한다. |
...
- 사용자 실사를 장기 보관하지 않으려면 실사 사용 환경에서 `DIRECT_FACE_MODE=false`를 사용해야 한다. OpenRouter의 요청 처리·보관 정책은 자체 스토리지 보관 정책과 별개로 확인한다.
...
| 삭제 규칙 | **✅ 정책 확정 (2026-09-16)** — 책 삭제 시 사용자 소유 레퍼런스·개인화 삽화·답변 MP3를 DB 레코드와 함께 제거. 탈퇴 시 모든 책에 적용 |
