...
상세 설계: [`docs/tasks/slice-3-face-session-spec.md`](slice-3-face-session-spec.md)
...
### Slice 7 — 리더 표면 통일 + 대화 분리 (프론트 전용, 착수 2026-09-14)
...
- [ ] ⚠️ **화면 회귀 확인** — `pnpm ci:core` 는 통과했지만 브라우저로 본 사람이 아직 없다.
      시연 리더가 이관 전과 같은지가 1순위다(같은 코드를 옮겼을 뿐이지만 **옮겼다는 사실 자체가 위험**이다).
- [ ] ⏳ 조합 선택 후 평가 옵션 회수
