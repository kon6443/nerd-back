...
- [x] **머지 후 정리 완료 (2026-09-04, 사용자 실행)** — 옛 시크릿 2개 삭제(`PROD` **9개** 확인) · 옛 stack 디렉터리 3개 삭제(`nerd-back` · `nerd-front` · `_deprecated_nerd_20260901`) · 옛 env 원본 4개 삭제. 서버에 남은 것은 `<DEPLOY_DIR>/{stacks,env}` 하나뿐이다. `tasks-stack-rename.md` 절차 11 도 이로써 닫혔다
  - **삭제 전 검증 완료 (2026-09-04)** — 새 트리의 env 사본 3개가 원본과 **바이트 동일**(`cmp` 종료코드 0)이고, 새 트리에 복사하지 않은 `nerd.prod.env` 에는 **고유한 키가 없다**(`comm -23` 출력 0줄). 옛 stack 디렉터리의 YAML·`init-users.sh` 는 전부 git 에 있어 재생성 가능하다 → **격리 없이 삭제해도 잃는 것이 없다**
  - ⚠️ env 원본을 지우면 **모노레포 머지를 되돌리는 경로가 사라진다**(옛 워크플로가 참조하던 시크릿·디렉터리가 함께 없어지므로). 배포·운영 확인이 끝났고 stack YAML 이 git 에 있어 재구성 가능하다는 판단으로 진행한다
  - ⚠️ 그 env 디렉터리에는 **다른 프로젝트의 숨김 파일**(`.env` · `.secrets-debug`)이 있다. `tree` 는 숨김 파일을 보여주지 않는다 — 와일드카드 대신 파일명을 명시한다
...
- [`lessons.md`](../lessons.md) 에 2026-09-03 로 **3건** 등재했다.
  1. 재시도가 도는 것과 재시도가 무엇을 시도하는지는 다른 문제다 (DB factory 멱등성)
  2. `bufferLogs: true` 는 부팅 로그의 타임스탬프를 전부 같게 만든다
  3. 「로컬 빌드 성공 ≠ 컨테이너 빌드 성공」 **재발** — 기존 2026-08-26 항목에 후속 한 줄로 덧붙였다(본문은 고치지 않았다)
