...
| **DB 에 쓰는 명령 실행** — `pnpm db:migrate:*`(**4개 전부**) · `pnpm db:seed:dev` · `pnpm db:sql` | 전 환경이 동일 DB — 실행이 곧 상용 적용. AI 는 파일 작성까지만, 실행은 사람이. 마이그레이션은 `DB_MIGRATION_*`(nerd_migrator), 시드는 앱 계정(DDL 이 아니라 행 쓰기라 그걸로 충분하다). ⚠️ `db:migrate:list` 도 포함이다 — `migration:show` 는 `migrations` 테이블이 없으면 `CREATE TABLE` 을 먼저 던진다(실측) |
