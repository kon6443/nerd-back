...
/** `animationend` 가 오지 않는 경우(탭 전환 등)의 안전장치. CSS 넘김 시간(620ms)보다 넉넉히 길다. */
 ...
 * 10 이면 곡선이 매끄럽다 — 8 이하에서는 휨을 키우는 순간 마디마다 꺾인 자국이 드러난다. 바꾸려면 `SEGMENT_BEND` 의 길이도 함께 맞춘다(명암은 마디 수와 무관하다 — `BookFrame.module.css`).
 */
const LEAF_SEGMENTS = 10;

/**
 * 마디가 몸통 회전에 **더 얹는** 휨(도). 앞이 책등 쪽이다. 실제 휨은 여기에 `--curl`(−1~1)을 곱한 값이다.
 *
 * ⭐ **끝으로 갈수록 커지는 경사다 — 얇은 종이의 모양.** 성경책 종이는 책등 쪽이 먼저 들리고 바깥 끝이
 * 늦게 끌려오다가 마지막에 앞질러 넘어간다(2026-09-14 7차 피드백: "두꺼운 종이 같다").
 * 누적 각도가 끝에서 +58° 가 되도록 `58° × (i+1) / 55` 로 나눴다 — 이웃 간 차이가 일정해야 꺾인 자국 없이 휜다.
 * 🚫 손으로 값을 흩뜨리지 않는다.
 *
 * 🔄 **합이 0 이 아니다(4차의 사인 활을 뒤집었다).** 합 0 은 끝이 늘 몸통과 같은 방향을 향해 판이 통째로
 * 도는 것처럼 보였다. 평평한 착지는 **착지 순간 `--curl` 이 0** 인 것으로 지킨다(`BookFrame.module.css`).
 *
 * ⚠️ **끝이 뒤처지는 양(58° × `--curl`)은 몸통 각도보다 작아야 한다.** 크면 종이 끝이 책 아래로 파고든다.
 * 그 제약은 몸통 각도와 `--curl` 을 **같은 키프레임**에 두고 지킨다 — 진폭을 바꾸면 키프레임도 다시 맞춘다.
 */
const SEGMENT_BEND = [
  "1.05deg",
  "2.11deg",
...
function LeafSegments({
  ...
  return (
    <div
      ...
      style={
{ "--bend": SEGMENT_BEND[depth] } as CSSProperties
      }
    >
      ...
      <div className={`${styles.book} ${fill ? styles.fill : ""}`}>
        ...
        <div className={styles.surface}>
          <div className={styles.spread}>
            ...
            {/* 쪽마다 스크롤 영역을 새로 만들어 다음 본문은 첫 줄에서 시작한다. */}
            <div key={baseText} className={styles.page}>
              {renderText(baseText)}
              {renderTextControls ? (
                <div className={styles.pageControls}>{renderTextControls(baseText)}</div>
              ) : null}
            </div>
          </div>
        </div>
...
/** 넘김 중에만 존재하는 종이. 모바일은 내용 한 벌만 합성하고 펼침면은 기존 곡면을 유지한다. */
export function BookTurn({
  turn,
  renderArt,
  renderText,
  onFinish,
}: Pick<BookPagerProps, "renderArt" | "renderText"> & {
  turn: Turn;
  onFinish: () => void;
}) {
  const single = turn.layout === "single";
  const singlePage = turn.direction === "next" ? turn.from : turn.to;
  return (
    <div
      aria-hidden="true"
      data-turn={single ? "next" : turn.direction}
      data-single={single ? turn.direction : undefined}
      className={`${styles.leaf} ${single ? styles.singleLeaf : ""}`}
      style={single ? undefined : { "--seg-count": LEAF_SEGMENTS } as CSSProperties}
    ...
    >
      {single ? (
        <>
          <div className={`${styles.singleFace} ${styles.faceStack}`}>
            <div className={styles.art}>{renderArt(singlePage)}</div>
            <div className={styles.page}>{renderText(singlePage)}</div>
          </div>
          <div className={`${styles.singleFace} ${styles.singleBack} ${styles.faceBlank}`} />
        </>
      ) : turn.direction === "next" ? (
        <LeafSegments
          depth={0}
