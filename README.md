...
**현재 상태**: 뼈대 계획 확정 · 구현 미착수. 아래 내용은 확정된 설계이며 코드는 아직 없다.
...
src/
├── common/
│   ├── constants/    decorators/    dto/         enums/
│   ├── filters/      guards/        logger/      pipes/
│   ├── port/         redis/         utils/       __spec__/
├── config/           env.validation.ts, app.config.ts
├── entities/         __spec__/entity.factory.ts
├── modules/
│   └── health/
├── app.module.ts
└── main.ts
...
- 필터는 3단으로 분기한다: 도메인 에러 DTO → 일반 `HttpException`(상태코드 매핑) → 그 외(`INTERNAL_SERVER_ERROR`, 원본 미노출).
...
- Mock 헬퍼: `src/common/__spec__/mock-repository.ts` — `createMockRepository()`, `createMockQueryBuilder()`
- 팩토리: `src/entities/__spec__/entity.factory.ts` — rosie + `@faker-js/faker`, 고정 시각 상수
- E2E는 **`AppModule`을 import하지 않는다.** `test/helpers/e2e-app.ts`의 헬퍼를 쓴다.
