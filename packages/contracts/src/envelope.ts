...
/**
 * 모든 API 의 경로 접두. **계약 값이라 여기가 SSOT 다** — 백엔드가 재수출하고 프론트가 그대로 쓴다.
 *
 * `api/v2` 인 이유: 지금은 이웃 프로젝트와 도메인을 공유하므로 경로 네임스페이스가 겹치지
 * 않아야 한다. "v1 의 다음 버전"이라는 뜻이 아니다.
 */
